...
export default function Post({ post, setPost }: PostProps) {
    ...
    async function loadPost() {
        const response = await axios.get(`http://localhost:3003/itens/${post.id}`)
        setPost((prev: Post[]) => (
            prev.map(prevPost => (
    ...
    async function handleCreateNewComment(event: FormEvent) {
        ...
        await axios.patch(`http://localhost:3003/posts/${post.id}`, {
            "comments": comments
        })
        setNewComment("")
        loadPost()
    ...
    async function handleLikeComment(event: MouseEvent, id: string) {
        event.preventDefault();
        const response = await axios.get(`http://localhost:3003/posts/${post.id}`)
